"""
Past_step_seven_v2 — Step 7b — Main habitat classification (id-based).

Reads the broad classification map produced by Step 7a, then for each broad_id
trains a separate Random Forest on main_id and applies it only to pixels
assigned to that broad_id.

Output pixel values ARE your main_id values directly (not re-encoded).

Must be run after Past_step_seven_v2/Step_7a_broad_classification.py.

Training data GeoJSON must have the fields:
    broad_id, main_habitat_type, main_id

Outputs (written to V2_OUTPUT_DIR):
    classification_main.tif                    — main habitat map (int16, values = your main_id)
...
def main():
    ...
    if not broad_tif.exists():
        raise FileNotFoundError(
            ...
            "Run Past_step_seven_v2/Step_7a_broad_classification.py first."
        )
    ...
    out_path = output_dir / "classification_main.tif"
