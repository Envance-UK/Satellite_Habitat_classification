"""
Past_step_seven_v2 — Step 7c — Vegetation structure classification (id-based).

Reads the main habitat classification map produced by Step 7b, then for each
main_id that has Vegetation_Structure training data, trains a separate Random
Forest on structure_id and applies it only to pixels assigned to that main_id.
...
Must be run after Past_step_seven_v2/Step_7b_main_classification.py.

Training data GeoJSON must have the fields:
    main_id, Vegetation_Structure, structure_id

Outputs (written to V2_OUTPUT_DIR):
    classification_vegetation_structure.tif     — structure map (int16, values = your structure_id)
...
def main():
    ...
    # ── Load main classification map from Step 7b ──────────────────────────────
    main_tif = output_dir / "classification_main.tif"
    if not main_tif.exists():
        raise FileNotFoundError(
            f"Main classification map not found: {main_tif}\n"
            "Run Past_step_seven_v2/Step_7b_main_classification.py first."
        )
    ...
    out_path = output_dir / "classification_vegetation_structure.tif"
