"""
Past_step_seven_cluster_membership_test — Step 7a — Veg/Non-Veg split, then
broad classification (id-based), with k-means cluster membership added as an
extra Random Forest feature.

Identical to Past_step_seven_v2/Step_7a_broad_classification.py, except the
...
statistics bands, letting the RF exploit the unsupervised cluster structure.
...
Outputs (written to CLUSTER_TEST_OUTPUT_DIR):
    classification_veg_nonveg_cluster.tif   — veg/non-veg map (1=Vegetation, 2=Non_Vegetation)
    classification_broad_cluster.tif        — broad classified map (int16, values = your broad_id)
...
def load_features(stats_tif, cluster_tif):
    ...
    X_stats   = data[:, valid_mask].T                    # (n_valid, n_stats_bands)
    X_cluster = cluster[valid_mask][:, np.newaxis]        # (n_valid, 1)
    X = np.hstack([X_stats, X_cluster])                   # (n_valid, n_stats_bands + 1)
...
def main():
    ...
    veg_tif = output_dir / "classification_veg_nonveg_cluster.tif"
